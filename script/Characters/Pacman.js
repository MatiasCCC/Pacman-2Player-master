class Pacman extends GameActors {
  ...
  setPointsIfEaten() {
    ...
    if (this.currentSoundToggle === 0) {
        this.audioLoader.play('eatingSound1', 1);
        this.currentSoundToggle = 1;
    } else {
        this.audioLoader.play('eatingSound2', 1);
        this.currentSoundToggle = 0;
    }
    // More dot consumption logic as before
    ...
    if (this.gameMap.getGamePointValueFromXY(this.tileFrom[0], this.tileFrom[1]) == ENERZIER_VALUE) {
	  ...
	  // Play the frightened sound once when the first ghost is set to frightened
